-- Labour Role Issue Diagnostic SQL Script
-- Run this in your PostgreSQL client to diagnose the labour entry role issues

\echo '====== LABOUR ENTRY ROLE DIAGNOSTIC ======'
...
\echo ''
\echo '3. RECENT LABOUR ENTRIES (LAST 20):'
\echo '======================'
SELECT
    le.id,
    le.site_id,
    ...
    u.username,
    r.role_name,
    CASE WHEN le.submitted_by_role = r.role_name THEN '✓' ELSE '❌ MISMATCH' END as match_status
...
\echo ''
\echo '4. CHECK FOR MISMATCHED SUBMITTED_BY_ROLE:'
\echo '======================'
SELECT
    le.id,
    le.labour_type,
...
\echo ''
\echo '5. DUPLICATE ENTRY CHECK (SAME ROLE, SAME LABOUR, SAME DATE):'
\echo '======================'
SELECT
    le.site_id,
    le.labour_type,
...
\echo ''
\echo '6. EXPECTED VS ACTUAL ROLES:'
\echo '====== For each recent entry, show if submitted_by_role matches user role ======'
