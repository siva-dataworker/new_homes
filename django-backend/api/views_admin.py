    ...
    try:
        ...
        return Response({
            'sites': [
                {
                    'id': str(s['id']),
                    'site_name': s['site_name'] if s['site_name'] else f"Site {str(s['id'])[:8]}",
...
# ============================================
# MISSING FUNCTIONS (Stubs for now)
# ============================================

@api_view(['POST'])
def update_site_metrics(request, site_id):
    """Update site metrics"""
    return Response({'message': 'Feature coming soon'}, status=status.HTTP_200_OK)


@api_view(['POST'])
def specialized_login(request):
    """Specialized login"""
    return Response({'message': 'Feature coming soon'}, status=status.HTTP_200_OK)


@api_view(['GET'])
def get_work_notifications(request):
...
@api_view(['POST'])
def upload_site_document(request, site_id):
    """Upload site document"""
    return Response({'message': 'Upload feature coming soon'}, status=status.HTTP_200_OK)
...
def get_site_dashboard(request, site_id):
    """
    Get live dashboard data for a specific site
    GET /api/admin/sites/{site_id}/dashboard/
    Returns: budget, allocated_budget, total_workers, total_bills, utilization_percentage
    """
    try:
        # Get allocated budget from site_budget_allocation
        allocated_budget_data = fetch_one("""
            SELECT 
                total_budget as allocated_budget
        ...
        allocated_budget = float(allocated_budget_data['allocated_budget']) if allocated_budget_data and allocated_budget_data['allocated_budget'] else 0
        
        # Get site budget (legacy)
        budget_data = fetch_one("""
            SELECT 
        ...
        total_budget = float(budget_data['total_budget']) if budget_data else 0
        
        # Use allocated budget if available, otherwise use legacy budget
        display_budget = allocated_budget if allocated_budget > 0 else total_budget
        
        # Get total workers (unique count from labour_entries)
        workers_data = fetch_one("""
            SELECT 
                COALESCE(SUM(labour_count), 0) as total_workers
            FROM labour_entries
            WHERE site_id = %s
        """, (site_id,))
        
        total_workers = int(workers_data['total_workers']) if workers_data else 0
        
        # Get total bills count
        bills_data = fetch_one("""
            SELECT 
        ...
        total_bills = int(bills_data['total_bills']) if bills_data else 0
        
        # Calculate total expenses from budget utilization summary
        utilization_data = fetch_one("""
            SELECT 
                total_spent,
        ...
        if utilization_data:
            total_expenses = float(utilization_data['total_spent']) if utilization_data['total_spent'] else 0
            utilization_percentage = float(utilization_data['utilization_percentage']) if utilization_data['utilization_percentage'] else 0
        else:
            # Fallback: Calculate from bills
            expenses_data = fetch_one("""
                SELECT 
            ...
            total_expenses = float(expenses_data['total_expenses']) if expenses_data else 0
            
            # Calculate utilization percentage
            utilization_percentage = 0
            if display_budget > 0:
                utilization_percentage = (total_expenses / display_budget) * 100
        
        return Response({
            'budget': display_budget,
            'allocated_budget': allocated_budget,  # NEW: Show allocated budget separately
